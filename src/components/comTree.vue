...
export default {
  ...
  mounted () {
    ...
    const userMtree = document.getElementById('userMtree')
    this.$nextTick(() => {
  ...
  methods: {
    ...
    renderContent (h, { node, data, store }) {
      return (
        <span class="custom-tree-node" style='display: flex'>
          <span class="tree_node_label">{data.label}</span>
    ...
    nodeDelete (node, data) {
      this.$emit('dellete', { node, data, next: () => this.deleteNext(node, data) })
    },
    deleteNext (node, data) {
